...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await getRuntimeAdapter(runtime.kind).restart(runtime.id);
    await writeAuditLog({ workspaceId: runtime.workspaceId, userId: user.id, action: "agent_runtime.restarted", entityType: "AgentRuntime", entityId: runtime.id, details: { kind: runtime.kind, success: result.success } });
