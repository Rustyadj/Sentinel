...
export async function PATCH(req: NextRequest, { params }: Context) {
  try {
    ...
    const user = current.workspaceId
      ? await requireWorkspacePermission(current.workspaceId, "task.update")
      : current.projectId
        ? (await requireProjectPermission(current.projectId, "task.update")).user
        : null;
...
export async function DELETE(_req: NextRequest, { params }: Context) {
  try {
    ...
    const user = task.workspaceId
      ? await requireWorkspacePermission(task.workspaceId, "task.delete")
      : task.projectId
        ? (await requireProjectPermission(task.projectId, "task.delete")).user
        : null;
    if (!user) return NextResponse.json({ error: "Task has no access scope" }, { status: 400 });
