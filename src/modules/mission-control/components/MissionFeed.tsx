...
import type { DataSourceState, FeedScope, MissionFeedItem } from "@/lib/mission-control/types";
import { selectMissionFeed } from "@/lib/mission-control/selectors";
import { cn } from "@/lib/utils";
import { MissionPanel, StatusDot, UnavailableState } from "./MissionPanel";
...
export function MissionFeed({ items, sourceState }: { items: MissionFeedItem[]; sourceState: DataSourceState }) {
  ...
  return (
    ...
    >
      ...
      {filtered.length === 0 ? <UnavailableState source={sourceState} emptyMessage="No events in this feed yet." /> : (
