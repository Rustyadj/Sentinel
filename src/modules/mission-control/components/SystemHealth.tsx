...
export function SystemHealth({ items }: { items: HealthItem[] }) {
  return (
    <MissionPanel title="System Health" action={<PanelLink href="/settings">Open monitor</PanelLink>} contentClassName="p-0">
      <ul className="divide-y divide-white/[0.07]" role="list">
        {items.map((item) => {
          const Icon = icons[item.id as keyof typeof icons] ?? Activity;
          const tone = item.status === "healthy" ? "positive" : item.status === "degraded" || item.status === "active" ? "warning" : "critical";
          return (
