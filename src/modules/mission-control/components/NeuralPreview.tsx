...
import type { NeuralPreviewData } from "@/lib/mission-control/types";
...
export function NeuralPreview({ data }: { data: NeuralPreviewData }) {
  return (
    <MissionPanel title="Neural Space Preview" action={<PanelLink href="/chat?space=graph">Open Neural Space</PanelLink>} contentClassName="p-0" className="h-full">
      <div className="grid md:grid-cols-[150px_1fr]">
        <div className="border-b border-white/[0.07] p-4 md:border-b-0 md:border-r">
          ...
          <div className="mt-3 text-[11px] font-medium text-white">{data.project}</div>
          <div className="mt-0.5 text-[9px] text-[#8290a2]">{data.workspace} workspace</div>
          <div className="mt-2 truncate text-[8px] text-[#718095]">{data.repository}</div>
          <div className="truncate text-[8px] text-violet-300">{data.branch}</div>
