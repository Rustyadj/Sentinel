...
import { Search, Bell, Lightbulb, MessageSquare, ChevronDown, Cpu, Zap } from "lucide-react";
  ...
  return (
    <header className="h-14 shrink-0 z-50 border-b border-[--border] bg-[--sidebar] flex items-center px-3 gap-3">
      ...
      <div className="flex items-center gap-1 shrink-0">
        {busyAgents.length > 0 && (
          <div className="hidden lg:flex items-center gap-1.5 rounded-full border border-amber-400/20 bg-amber-400/10 px-2.5 py-1 text-[10px] font-medium text-amber-300">
            <Zap className="h-3 w-3 animate-pulse-dot" />
        ...
        <button
          title="Quick actions"
          onClick={() => setCommandBarOpen(true)}
          className="hidden md:flex h-8 items-center gap-1.5 rounded-lg border border-[--primary]/20 bg-[--primary] px-3 text-xs font-medium text-white shadow-[0_0_24px_rgba(124,108,246,.18)] transition-colors hover:bg-[--primary]/90"
        >
