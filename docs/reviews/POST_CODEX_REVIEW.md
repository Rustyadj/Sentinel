...
**Regression test added:** `tests/mission-control/server.test.ts`, three DB-backed integration tests — (1) a learning candidate on a project outside the top-20 accessible-project list still appears in `attention`, (2) `costToday` is exact across 240 same-day experiences for one agent, (3) the currently in-progress experience is still found as `currentTask` alongside other same-day rows. All three pass against a real Postgres.

Verified again after this revision: `npx tsc --noEmit` clean, `npx eslint src/lib/mission-control/server.ts` clean, `npm run build` succeeds, `npm test` 115/115 passing (112 pre-existing + 3 new).
