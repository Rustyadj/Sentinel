...
- Reviewed PR #20 head: `fd8329ee03d63f5ca5bd7bacba9efd00a2530a68`
- Initial reviewed head before Claude's subsequent commits: `5d5eeea65ef5f4be50b8bb967dd06dfa81aa8398`
- `origin/main`: `8197cd4a8b08088e02644d03e7ceba6baca24345`
- Merge base: `8197cd4a8b08088e02644d03e7ceba6baca24345`
- Final reviewed PR delta: 99 files, 10,428 insertions, 116 deletions across 12 commits.
- Migrations reviewed:
  - `20260802002232_learning_core_on_neural_engine`
  - `20260802024239_rollback_skill_version_links`
- In scope: all 12 PR commits; schema/migrations; every `/api/learning/*` route; Neural Engine candidate review/apply/rollback; policy, sandbox, shadow, benchmark, feature-flag, queue/worker, Learning Core UI; PR #17 unique-value review; PR #8 deployment review.
...
The audit ran in `/home/rusty/Development/Sentinel-release-audit`. Claude's live checkout and branch were not modified. Claude's subsequent commits were merged without rewriting their history; two comment-only conflicts in queue/worker files were resolved by retaining both their live-verification record and the audit's retry/shutdown hardening.

## Severity findings

### Critical

1. **Authenticated cross-tenant reads remain widespread.** Authentication is present, but many routes read global tables without workspace/project/user predicates: candidates, curiosity, reflections, goals, knowledge gaps, replay, benchmark definitions, feature flags, skills/versions, scheduler status, improvement queue and timeline. An authenticated user can enumerate other tenants' candidates, reflections, approvals, flags, benchmark definitions and operational history. The Overview route was tenant-scoped in `fd8329e` and is regression-tested on this audit branch. Several underlying models (`Skill`, `TrustEvent`, `AgentCompetency`, replay runs) lack direct tenant ownership, so safe remediation needs a deliberate read-model/schema design rather than a cosmetic `requireUser()` check.
...
- Preserved and regression-tested Claude's final Overview tenant scoping so an authenticated outsider cannot see another workspace's Learning Events.
...
| `tests/release-audit/learning-api-authorization.test.ts` | Outsider cannot read another workspace's Learning Events in Overview, mutate its flag/reflection, read agent trust, inject benchmark results, run shadow work, or roll back a candidate. |
...
|---|---|---|---|
| `b4585c6` | Phase A event/redaction foundation, UI shell and 451-line migration; extends Experience/LearningCandidate/Skill and adds Learning Core tables. | Adds redaction and audit integration; expands chat capture. | Neural Engine is extended rather than replaced. Migration contains unrelated index drop; Overview was initially global and was fixed in `fd8329e`. **High**. |
...
| `fd8329e` | Scopes Overview counts, metrics, activity and trust queries to workspaces accessible to the current user. | No test in the source branch; this audit adds an adversarial outsider regression. | Correctly closes the proven Overview disclosure without claiming to solve the remaining global read APIs. **Low** residual. |
